...
/*
 * php5 class (will not work in php4) for detecting bitrate and duration of regular mp3 files (not VBR files)
 */
...
class mp3file
{
    ...
    protected function readmp3frame()
    {
        ...
        if ($iscbrmp3) {
            ...
            $this->mp3data['Length hh:mm:ss'] = mp3file::seconds_to_hhmmss($this->mp3data['Length']);
    ...
    protected function containsvbrxing()
    {
        ...
        /*
         * || ($this->block[21]==88 && //X 0x58 $this->block[22]==105 && //i 0x69 $this->block[23]==110 && //n 0x6E $this->block[24]==103) //g 0x67
         */
        ;
    }
    ...
    protected function getnextbit()
    {
        ...
        if ($this->bitpos == 8) {
            ...
            if ($this->blockpos == $this->blockmax)             // end of block reached
            {
                ...
                if ($this->blockpos == $this->blocksize) { // end of short block reached (shorter than blockmax)
                    return; // eof
                }
    ...
    public static function bitratelookup(&$mp3)
    {
        ...
        if (mp3file::is_mpeg10($mp3) && mp3file::is_layer1($mp3)) // V1,L1
            ...
            if (mp3file::is_mpeg10($mp3) && mp3file::is_layer2($mp3)) // V1,L2
                $whichcolumn = 1;
            else 
                if (mp3file::is_mpeg10($mp3) && mp3file::is_layer3($mp3)) // V1,L3
                    $whichcolumn = 2;
                else 
                    if (mp3file::is_mpeg20or25($mp3) && mp3file::is_layer1($mp3)) // V2,L1
                        $whichcolumn = 3;
                    else 
    ...
    public static function seconds_to_hhmmss($duration){
        ...
        //$nHours = floor($duration /3600);
        return gmdate("H:i:s", $duration);
        
    	//return sprintf("%h:%d:%02d",$nHours ,($duration / 60), $duration % 60);
    }
}
?>